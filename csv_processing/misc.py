...
def get_preceding_char(word, pos):
    return word[pos - 1] if pos > 0 else ' '

def get_following_char(word, pos):
    return word[pos + 1] if pos < len(word) - 1 else ' '
...
def preprocess_data(file_path):
    ...
    # Add preceding and following characters
    data['Preceding_Char'] = data.apply(lambda row: get_preceding_char(row['Original_Word'], row['Missing_Letter_Position']), axis=1)
    data['Following_Char'] = data.apply(lambda row: get_following_char(row['Original_Word'], row['Missing_Letter_Position']), axis=1)
...
def run_logistic_regression(data, dataset_name):
    ...
    formula = 'Top1_Is_Accurate ~ is_vowel + Word_Length + Normalized_Missing_Letter_Position + Num_Vowels + Word_Length*Num_Vowels + Preceding_Char + Following_Char'
    ...
    X = pd.concat([X, pd.get_dummies(data['Preceding_Char'], prefix='Preceding_Char'), pd.get_dummies(data['Following_Char'], prefix='Following_Char')], axis=1)
